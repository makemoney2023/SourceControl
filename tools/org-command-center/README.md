...
| Knowledge graph | Intelligence menu → live org graph for the **active customer** (initiatives, seats, handoffs, runs, artifacts). Breadcrumb up to agency (structure only). Drill Agency → Customer → Initiative → Seat. Obsidian edges from `memorybank/org/GRAPH/` MOCs + work-note footers. Graphify CLI remains the code/AST map. |
