...
**Phase:** 5  
**Status:** operator-approved — strategy-to-spec lock (C-suite yes/no pending; RUNBOOK phase advance held)  
**Last updated:** 2026-08-05 (run `1785959774541-head-of-product` — operator PRD approval + pre-build interview scheduled)  
...
Blacksage Kennels needs a **trust-first public website** that lets serious ADRK-aligned Rottweiler buyers **evaluate program credibility before initiating inquiry** — not a cosmetic reskin of the rejected v1 prototype. This PRD defines what ships in production rebuild (Phase 9), how staged launch tiers adapt to operator program maturity, and acceptance criteria covering **all four v1 failure layers** (visual, experiential/3D, trust/content, UX/conversion).

**Operator update (2026-08-05):** Q1 resolved — **active placement program** (20+ years operating, ≥1 litter/year). Default packaging is **Package B (waitlist/inquiry)** with **Litters** in navigation when litter facts are verified. Public launch remains gated on inquiry email (Q7), health inventory, and photography delivery (target 2026-08-31).

**Operator approval (2026-08-05, run `1785959774541-head-of-product`):** Operator **approved this PRD as the strategy-to-spec lock** for the trust-first kennel website rebuild. Tier branches remain valid; **public launch cannot proceed** until remaining operator gates close (see Pre-build operator interview). **RUNBOOK phase advance held** until C-suite yes/no on head-of-product brief.
...
| **Packaging** | A Interest list / B Waitlist / C Placement — **Package B default** (Q1 resolved: active program) |
...
### 12-month metrics (operator Q4 — resolved 2026-08-05)
...
| M3 Inquiry process documented | Destination + owner + response SLA | Yes (Q7 — **still blocked**) |
...
| **M9 Organic search visibility** | **Rank for priority breeder/Rottweiler queries in Ontario + North America shipping context** | **No — operator-defined (Q4)** |
| **M10 AI search / answer-engine presence** | **Blacksage retrievable in AI-assisted breeder research with accurate, citeable facts** | **No — operator-defined (Q4)** |
| **M11 Technical SEO baseline** | Indexable pages, valid structured data, Core Web Vitals pass on launch tier | No |
...
| **About** | Operator identity, program philosophy, geography/contact | **Geography resolved:** Collingwood & Beaverton, Ontario; ships throughout North America. Phone/email when Q7 closes |
...
### Conditional sections (Q1 resolved — active program)

| Section | When live | When hidden / alternate |
|---------|-----------|-------------------------|
| **Litters / Puppies** | **Default ON** when operator confirms litter facts for current cycle | Honest "no upcoming litter" copy when none planned |
| **Waitlist application (Package B)** | **Default ON** — active program with qualified inquiry path | Revert to Package A only if operator pauses breeding program |
| **Deposit language** | Generic process copy approved (see Placement process) | **No dollar amounts** on site |
...
## Packaging map (A / B / C) — Q1 resolved

Three packages — **do not collapse**. **Default live UX: Package B** (Q1 = active program, 20+ years, ≥1 litter/year).

| Package | Trigger | Site UX | Payment on site |
|---------|---------|---------|-----------------|
| **A — Interest list** | Pre-litter / program pause fallback | Email + brief interest capture | **None** |
| **B — Waitlist** | **Active program (default)** | Full application fields; copy states deposit **after** approval, off-site | **None** |
| **C — Placement** | Waitlist match + contract | Process description only; contract terms off-site | **None** — all off-site |
...
Launch tier selected by **Q1** (resolved: active program) and **Q6** (photography timeline: target 2026-08-31). Both tiers must pass all four v1 failure-layer AC.

### Tier 2 — Active program (default — Q1 resolved 2026-08-05)

**Minimum live sections:** All primary IA sections including Dogs (+ Litters when litter facts verified)  
**Conversion:** **Package B** (waitlist/inquiry) default; Package C described in process copy only  
**Photography:** Operator-supplied kennel/dog photos required before public launch (`FR-VS-002`); shoot target **2026-08-31**  
**Health:** Per-dog clearance links only when operator inventory exists — **inventory still pending**  
**Geography:** Collingwood & Beaverton, Ontario; ships throughout North America (publishable now)

### Tier 1 — Brand-first (fallback only)

**Use when:** Operator pauses active program or photography/health gates cannot close in time  
**Minimum live sections:** Home, About, Health/Education, Contact/Interest list  
**Dogs:** Honest empty state — **not** fake dog profiles  
**Litters:** Omitted or explicit coming-soon  
**Conversion:** Package A (Interest list) only  
**Photography:** Brand/kennel/environment only; no stock dogs as Blacksage stock
...
Tier 0 (staging)
  ...
  → Tier 1 fallback when: photography or health inventory delayed beyond target launch
```
...
| Decision ID | Status | Answer / default | Unblocks | Launch tier impact |
|-------------|--------|------------------|----------|-------------------|
| **Q1** | **Resolved** | Active program — 20+ years, ≥1 litter/year | Litters nav, Package B, Tier 2 default | **Tier 2** |
| **Q2** | **Partial** | Collingwood & Beaverton, ON; ships North America | About/footer geography; partial LocalBusiness | Tier 2 partial — full NAP when phone/email added |
| **Q4** | **Resolved** | 12-month success = search engine + AI search ranking | M9–M11 metrics; SEO AC weight | Both tiers |
| **Q6** | **Partial** | Shoot target **2026-08-31**; assets pending | Hero, Dogs photography | Tier 2 public launch blocked until delivered |
| **Q7** | **Blocked** | Inquiry email TBD | Form routing, SLA copy, soft launch | **No public launch** |
| **Q8** | **Needs follow-up** | Operator requested plain-English rephrase | Scope sizing | Default: Phase 4 trust-first static scope (~$5.5k band) |
| **Health inventory** | **Blocked** | Per-dog tests unknown | Named dogs, OFA/CHIC links | Dogs page population |
| **GAP-CL** | **Resolved** | TNRC, CKC, AKC, Rottweiler Club of Canada | About affiliation badges | Tier 2 |
| **GAP-NT** | **Resolved** | Natural tails; no docking | Health/Education breed-standard copy | Both tiers |
| **GAP-CT** | **Resolved (draft)** | Selective placement process copy (see below) | Process transparency | Both tiers |
| **OP-P1/P2** | Open | Price/deposit off-site only | Generic qualification copy | Both tiers — no amounts on site |

### Verified affiliations (publishable on About)

- True North Rottweiler Club  
- The Canadian Kennel Club  
- The American Kennel Club  
- The Rottweiler Club of Canada  

### Natural tail policy (publishable)

Blacksage Rottweilers are bred and placed with **natural tails; tail docking is not performed**. Education copy may reference ADRK/FCI natural-tail standard alignment.

### Placement process copy (operator-delegated draft — 2026-08-05)

1. **Inquiry** — Submit after reviewing on-site program evidence.  
2. **Mutual-fit review** — Blacksage evaluates experience, goals, and home fit.  
3. **Waitlist consideration** — Approved applicants may join waitlist; deposit terms provided individually off-site.  
4. **Contract & placement** — Written agreement before deposit; health and rehome terms per contract.  
5. **Lifetime support** — Rehome/right-of-first-refusal posture per selective placement norms.

**Critical path (updated):** Q7 (email) → Q6 (photography by 2026-08-31) → health inventory → operator sign-off (`AC-HN-005`).

---

## Pre-build operator interview (scheduled 2026-08-05)

Operator authorized scheduling. Interview closes remaining launch gates **before build resume** — PRD tier branches are locked; public launch still blocked until outputs land.

| Field | Value |
|-------|-------|
| **Purpose** | Close Q7, Q8, health inventory, Q6 delivery plan, Q2 phone, operator bio, current litter facts |
| **Proposed window** | **2026-08-12 – 2026-08-14** (operator to confirm slot) |
| **Duration** | 45–60 minutes |
| **Facilitator** | head-of-product (or orchestrator on operator request) |
| **Prerequisite** | C-suite yes/no on HoP brief — **RUNBOOK does not advance until C-suite decides** |

### Interview agenda (load-bearing)

| # | Topic | Closes | Prepare |
|---|-------|--------|---------|
| 1 | Inquiry email, owner, response-time expectation | Q7, M3, soft-launch minimum | Monitored inbox ready to paste |
| 2 | Budget ceiling + target public launch date (plain English) | Q8, scope sizing | Rough max spend; date preference |
| 3 | Kennel photography — shoot date vs 2026-08-31 target | Q6, `FR-VS-002` | Confirm shoot scheduled or slip plan |
| 4 | Health-test inventory per named breeding dog | Health inventory, Dogs page | List: dog name → test → registry link |
| 5 | Public phone number (optional) | Q2 remainder, tap-to-call, LocalBusiness | Yes/no + number |
| 6 | Operator bio / kennel story (2–3 paragraphs) | About page, GAP-OP | Draft or bullet points |
| 7 | Current/upcoming litter facts (if publishing Litters) | Litters page honesty | Parents, timing, availability posture |
| 8 | Placement process copy sign-off | GAP-CT final | Review draft in Operator Decision Register |

**Exit criteria:** Updated Operator Decision Register; `OPERATOR-LAUNCH-BLOCKERS.md` refreshed; soft-launch minimum checklist actionable.
...
| `NFR-SEO-002` | Structured data | JSON-LD Organization + **LocalBusiness when Q2 phone/email confirmed**; region publishable now (Collingwood/Beaverton ON) |
...
- Operator story, program principles (20+ years operating — verified)  
- Geography: Collingwood & Beaverton, Ontario; ships throughout North America  
- Club affiliations: TNRC, CKC, AKC, Rottweiler Club of Canada (**verified 2026-08-05**)  
- Contact methods when Q7 closes (phone optional when operator provides)
...
| ID | Status | PRD impact |
|----|--------|------------|
| **Q1** | **Resolved** | Tier 2 + Package B default |
| **Q2** | **Partial** | Geography live; phone/email pending |
| **Q4** | **Resolved** | M9–M11 SEO/AI search success metrics |
| **Q6** | **Partial** | Target shoot 2026-08-31; Dogs/hero gated |
| **Q7** | **Blocked** | Form routing, SLA copy, soft launch |
| **Q8** | **Needs follow-up** | Budget/timeline ceiling — plain-English ask pending |
| **Health inventory** | **Blocked** | Per-dog health claims on Dogs |
| **OP-P2** | Open | Deposit policy copy only — no amounts on site |
...
- [x] Operator Decision Register — operator answers merged 2026-08-05; remaining gaps labeled  
- [x] NFR traceability merged from business-analyst  
- [x] **Operator PRD approval** — strategy-to-spec lock (2026-08-05)  
- [x] Pre-build operator interview scheduled (2026-08-12 – 2026-08-14 proposed)  
- [ ] C-suite review complete (yes/no — **RUNBOOK phase advance held until decided**)  
...
| business-analyst | `HANDOFFS/5-business-analyst.md` | RTM, Operator Decision Register, NFR/AC, tier rules, `/inquire` routing; **operator answers applied 2026-08-05**; **pre-build interview agenda 2026-08-05** |
